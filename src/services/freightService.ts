  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (extension === 'csv') {
      ...
      try {
        ...
        const chunksResults = [];
        // Processar em lotes de 4 chunks por vez
        const batchSize = 4;
...
export const performAudit = (dataA: CTEData[], dataB: CTEData[], tolerance: number = 0): AuditResult[] => {
  ...
  allCtes.forEach(cte => {
    ...
    if (itemA && itemB) {
      matchedB.add(cte);
      results.push(createAuditResult(cte, itemA, itemB, tolerance));
    }
  });
...
const createAuditResult = (cte: string, itemA: CTEData, itemB: CTEData, tolerance: number = 0): AuditResult => {
  ...
  const isDivergent = diffEmpresaRounded > tolerance || diffMotoristaRounded > tolerance;
  ...
  return {
    ...
    divergencias: {
      freteEmpresa: diffEmpresaRounded > tolerance ? diffEmpresaRounded : undefined,
      freteMotorista: diffMotoristaRounded > tolerance ? diffMotoristaRounded : undefined,
      margem: diffMargemRounded > tolerance ? diffMargemRounded : undefined,
      peso: diffPesoRounded > tolerance ? diffPesoRounded : undefined,
    }
  };
};
...
export const exportToExcel = (results: AuditResult[]) => {
  const exportData = results.map(r => ({
    'CTE': r.cte,
    'Status': r.status === 'A_ONLY' ? 'Apenas Relatório Sistema A' : 
              r.status === 'B_ONLY' ? 'Apenas Relatório GW' : 
...
export const shareToWhatsApp = (results: AuditResult[], summary: AuditSummary) => {
  ...
  if (issues.length > 0) {
    ...
    issues.slice(0, 10).forEach(r => {
      if (r.status === 'A_ONLY') text += `- CTE ${r.cte}: Faltante no Relatório GW\n`;
      else if (r.status === 'B_ONLY') text += `- CTE ${r.cte}: Faltante no Relatório Sistema A\n`;
...
export const calculateSummary = (results: AuditResult[], footerTotalA?: number, tolerance: number = 0): AuditSummary => {
